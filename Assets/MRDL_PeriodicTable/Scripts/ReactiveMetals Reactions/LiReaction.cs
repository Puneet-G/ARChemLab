...
public class LiReaction : MonoBehaviour
{
    bool shrink = false;
    bool react = false;
    ...
    float speed = (2 * Mathf.PI) / 8; //2*PI in degress is 360, so you get 2 seconds to complete a circle
    float radius = 0.05f;
    ...
    void Update()
    {
        ...
        if (react)
        {
            ...
            {
                react = false;
                Destroy(reactionEffect);
            }
            else
                this.transform.localScale -= (Time.deltaTime/4)*originalScale;
        }
    }
